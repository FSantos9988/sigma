 ...
 * --------------------------------------------------------------------
 * ZnetDK Input multifiles widget
 *
 * File version: 1.2
...
$(function () {
    $.widget("znetdk.zdkmultiupload", {
        options: {
            ...
            photoReductionPercent: 100, // Photo reduction in percent
            minPhotoSizeForReduction: 800 // Minimal size in pixels of the photo to reduct it
        },
        /**
        ...
        _setOptionsFromAttributes: function () {
            var actionAttrib = znetdk.getActionFromAttr(this.element),
                ...
                photoReductPerc = znetdk.getTextFromAttr(this.element,'data-zdk-photoreductpercent'),
                minPhotoSizeForReduct = znetdk.getTextFromAttr(this.element,'data-zdk-minphotosizetoreduct');
            ...
            if (photoReductPerc !== false) {
                this.options.photoReductionPercent = parseInt(photoReductPerc);
            }
            if (minPhotoSizeForReduct !== false) {
                this.options.minPhotoSizeForReduction = parseInt(minPhotoSizeForReduct);
            }
        },
        /**
        ...
        /**
         * Set the selected filed matching the specified position into the file
         * container as an hyperlink for download
         * @param {integer} position Position of the file into the file container
         * @param {string} url Url for downloadling the attachment
         * @return {boolean} Returns true if the hyperling has been set successfully
         */
        _setSelectedFileAsLink: function(position, url) {
            var fileContainer = this.selectedfileContainer.children('div').eq(position);
            if (fileContainer.length === 0) {
                return false;
            }
            fileContainer.find('span.zdk-multiupload-filename').wrap('<a href="'
                + url + '" target="_blank"></a>');
            return true;
        },
        /**
        ...
        _selectNewFile: function() {
            ...
            newInputfile.appendTo(this.inputfileContainer).zdkinputfile({
                ...
                showThumbnail:false,
                noFileLabel:'',
                photoReductionPercent: this.options.photoReductionPercent,
                minPhotoSizeForReduction: this.options.minPhotoSizeForReduction
            });
        ...
        setUploadedFiles: function(files) {
            ...
            $.each(files, function(index, value){
                ...
                if (value.url !== undefined && value.url.length > 0) {
                    $this._setSelectedFileAsLink(index, value.url);
                }
            });
            return isOk;
        },
