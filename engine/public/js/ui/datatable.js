...
$(function () {
    $.widget("znetdk.zdkdatatable", $.primeui.puidatatable, {
        ...
        options: {
            ...
            resizableColumns: false, /* Columns can be resized by the user with the mouse */
            resetSortLabel: null /* Label of the button for restoring to the default sort order */
        },
        /**
        ...
        _create: function () {
            ...
            this._setResetSortLabelFromAttribute(); // Label of the button displayed for resetting the sort order 
            this._getNumberOfPaginatorRowsFromLocalStorage(); // The locally stored paginator 'rows' option is read if exists
            this._addHeaderAndBodyClasses(); // The syling class 'zdk-col-xxxx' are added to the table cells
            this._allowHtmlForColumns(); // Columns with isHtml property are filled in HTML
        ...
        _getRemoteDataSource: function () {
            return function (callback_function, ui) {
                ...
                if (ui.sortField && ui.sortOrder) { // sort parameters
                    ...
                    this._showHideResetSortButton(true);
                }
        ...
        /**
         * Enables the multiple selection of rows
         */
        ...
        /**
         * Sets the button label for resetting the datatable sort order
         */
        _setResetSortLabelFromAttribute: function() {
            var buttonLabel = znetdk.getTextFromAttr(this.element, 'data-zdk-resetsortlabel');
            if (buttonLabel !== false) {
                this.options.resetSortLabel = buttonLabel;
                // The 'data-zdk-resetsortlabel' attribute is removed because it is no longer usefull
                this.element.removeAttr('data-zdk-resetsortlabel');
            }  
        },
        /**
        ...
        _getNumberOfPaginatorRowsFromLocalStorage: function() {
            ...
            try {
                var storedValue = znetdk.getLocalSettings(storageKey);
                if(storedValue) {
        ...
        /**
         * Returns the row data matching the specified datatable row element
         * @param {jQuery} row The HTML row element (i.e <tr>) as jQuery object
         * @returns {Array} A table containing the row data
         */
        getRowData: function(row) {
            var rowIndex = this._getRowIndex(row);
            return typeof this.data[rowIndex] === 'undefined' ? null : this.data[rowIndex];
        },
        /**
        ...
        _makeColumnsResizable: function() {
            ...
            var $this = this,
                thHeight = this.thead.find('th:first').height();
            this.thead.find('th').resizable({
                ...
                resize: function (event, ui) {
                  ...
                  $this._storeLocallyNewColumnWidth(ui.element.index(), ui.size.width);
                }
            }); 
        },
        _storeLocallyNewColumnWidth: function(headingCellIndex, columnWidth) {
            var columnName = this.options.columns[headingCellIndex].field,
                storageKeyPrefix = 'zdkdatatable_' + this.element.attr('id') + '_column_width_',
                storageKey = storageKeyPrefix + columnName;
            znetdk.storeLocalSettings(storageKey, columnWidth);
        },
        _setColumnWidthsFromLocalStorage: function() {
            var storageKeyPrefix = 'zdkdatatable_' + this.element.attr('id') + '_column_width_';
            for(var j = 0; j < this.options.columns.length; j++) {
                var columnOptions = this.options.columns[j],
                    columnWidth = znetdk.getLocalSettings(storageKeyPrefix + columnOptions.field);
                if (columnWidth !== false) {
                    this.thead.find('th.' + columnOptions.headerClass).width(columnWidth);
                }
            }
        },
        ...
        _getResetSortButton: function() {
            if (this.options.resetSortLabel === null) {
                return false; // No label set for the button...
            }
            var existingButtonElement = this.element.find('.zdk-datatable-resetsortbutton');
            if (existingButtonElement.length === 1) {
                return existingButtonElement; // The button already exists
            }
            // The button does not exist and so it is created
            this.element.children('.pui-datatable-caption')
                .after('<button class="zdk-datatable-resetsortbutton" type="button">'
                + this.options.resetSortLabel + '</button>');
            var newButtonElement = this.element.find('.zdk-datatable-resetsortbutton');
            newButtonElement.puibutton({icon:'fa-sort-alpha-asc'});
            this._handleResetSortButtonClick(newButtonElement);
            return newButtonElement;
        },
        _showHideResetSortButton: function(isVisible) {
            var buttonElement = this._getResetSortButton();
            if (buttonElement === false) {
                return false; // No label is set, feature unvailable...
            }
            if (isVisible === false) { // Is hidden...
                buttonElement.hide();
            } else { // Is visible...
                buttonElement.show();
            }
        },
        _handleResetSortButtonClick: function(buttonElement) {
            var $this = this;
            buttonElement.on('click.zdkdatatable', function(event) {
                event.preventDefault();
                // The column header sort state is reset
                var currentSortedColumn = $this.thead.children('th.pui-sortable-column.ui-state-active');
                currentSortedColumn.data('order', 0).removeClass('ui-state-active')
                    .children('span.pui-sortable-column-icon').removeClass('ui-icon-triangle-1-n ui-icon-triangle-1-s');
                // The datatable content is refreshed with default sort order
                $this.options.sortField = null;
                $this.options.sortOrder = null;
                $this.refresh();
                // The button is hidden
                $this._showHideResetSortButton(false);
            });
        },
        _addHeaderAndBodyClasses: function() {
            for (var j = 0; j < this.options.columns.length; j++) {
                this.options.columns[j].headerClass = 'zdk-col-' + this.options.columns[j].field;
                this.options.columns[j].bodyClass = 'zdk-col-' + this.options.columns[j].field;
            }
        },
        /**
         * For columns with isHtml property, the HTML parsing is allowed 
         */
        _allowHtmlForColumns: function() {
            for (var j = 0; j < this.options.columns.length; j++) {
                if (this.options.columns[j].hasOwnProperty('isHtml')
                        && this.options.columns[j].isHtml === true) {
                    const field = this.options.columns[j].field;
                    this.options.columns[j].content = contentInHtml;
                    function contentInHtml(rowData) {
