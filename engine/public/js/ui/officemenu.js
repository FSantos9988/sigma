...
$.widget("znetdk.zdkofficemenu", {
    ...
    _create: function () {
        ...
        this.element.find('ul a > span.fa').addClass('fa-lg');
        ...
        var menu = $('<ul/>').appendTo(winMgrElement),
            dataIcon = this.options.winAutoClose ? 'fa-check' : 'ui-icon-blank';
        menu.append('<li><h3>' + titleLabel + '</h3></li>');
        this.autoCloseItem = $('<li><a data-icon="' + dataIcon + '">' + autoCloseLabel + '</a></li>').appendTo(menu);
        this.adjustHorizItem = $('<li><a data-icon="fa-ellipsis-h">' + adjustHorizLabel + '</a></li>').appendTo(menu);
        this.adjustVerticItem = $('<li><a data-icon="fa-ellipsis-v">' + adjustVerticLabel + '</a></li>').appendTo(menu);
        this.closeAllItem = $('<li><a data-icon="fa-times">' + closeAllLabel + '</a></li>').appendTo(menu);
            ...
            this.menuWidget.find('ul > li > a').click(function (event) {
                if ($(this).next('ul').length === 0) {
                    var menuItemId = ($(this).attr('href')).substr(1),
                        ...
                        menuItemIcon = $(this).data('icon');
                    $this._openWindow(menuItemId, menuItemLabel, menuItemIcon);
                }
     ...
     * 'initview' event
     */
    _openWindow: function (menuItemId, menuItemLabel, menuItemIcon, callParameter) {
        var $this = this,
        ...
        if (dlgIsCreated) {
            ...
            dialogElement = $('<div id="' + dialogId + '" title="' + menuItemLabel + '" data-icon="'
                + menuItemIcon + '"/>');
            ...
            znetdk.loadView({
                ...
                callback: function () {
                    var isVisible = true,
                        ...
                        isAutoHeight = dialogElement.find('.zdk-view-maxheight').length > 0;
    ...
    openWindow: function (viewID, callParameter) {
        if (this.menuWidget.length === 1) {
            var menuItemId = viewID === undefined ? this._getFirstChildMenuItemId() : viewID,
                menuItemLabel, menuItemIcon;
            menuItemLabel = this.menuWidget.find('ul > li > a[href="#' + menuItemId + '"] > span.ui-menuitem-text').text();
            menuItemIcon = this.menuWidget.find('ul > li > a[href="#' + menuItemId + '"]').data('icon');
            if (menuItemLabel) {
                this._openWindow(menuItemId, menuItemLabel, menuItemIcon, callParameter);
            }
        }
    },
    /**
    ...
    /**
     * Refresh the color of the menu icons when the widget theme is changed on the fly
     */
    refreshIconsColor: function() {
        this.menuWidget.children('ul').first().puitieredmenu('refreshFaIconColor');
    }
});
