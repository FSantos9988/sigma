 ...
 *   - When the 'data-icon' is set for the element, the specified fontawesome
 *     icon is displayed before on the left of the dialog title
 * File version: 1.8
...
$(function () {
    $.widget("primeui.puidialog", $.primeui.puidialog, {
        ...
        _create: function () {
            ...
            // ZnetDK extension : icon is added to the title bar
            this._setTitleBarIconFromAttribute();
        ...
        _setSize: function () {
            ...
            if (this.options.appendTo && !this.options.autoHeight) {
                this.content.css('max-height', this.options.appendTo.height() - 50);
            }
        },
        _setAutoHeight: function() {
            var windowHeight = $(window).height(),
                dialogPosition = this.element[0].getBoundingClientRect(),
                dialogHeight = windowHeight - dialogPosition.top - 5;
            this.element.height(dialogHeight);
            this._setContentAutoHeight();
        },
        /**
         * Calculate the dialog content height according to the dialog height
         * and the title bar height
         * @returns {float} The calculated content height
         */
        _setContentAutoHeight: function() {
            var dialogOuterHeight = this.element.outerHeight(true),
                titleBarOuterHeight = this.titlebar.outerHeight(true),
                contentOuterHeight = this.content.outerHeight(true),
                contentHeight = this.content.height();
            this.content.height(dialogOuterHeight - titleBarOuterHeight
        ...
        /**
         * Adds the fontawesome icon specified through the 'data-icon' attribute
         * to the title bar, on the left of the text title.
         * @returns {Boolean} Value true when the 'data-icon' attribute exists,
         * false otherwise.
         */
        _setTitleBarIconFromAttribute: function() {
            var titleBarIcon = this.element.data('icon');
            if (typeof titleBarIcon === "undefined") {
                return false;
            }
