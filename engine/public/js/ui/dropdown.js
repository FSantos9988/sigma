...
$(function () {
    $.widget("znetdk.zdkdropdown", $.primeui.puidropdown, {
        ...
        selectValue : function(value) {            
            var selectElName = typeof this.element === 'undefined' ? '????' : this.element.attr('name');
            if (this.options.loadOnCreate !== 'yes' && (
                    typeof this.choices === 'undefined' 
                    || this.choices.length === 0)
                    ) {
                this._loadData(value);
            } else if (typeof this.choices !== 'undefined' 
                    && this.choices.length > 0) {
                var option = this.choices.filter('[value="' + value + '"]');
                if (option.length === 1) {
                    this._super(value);
                } else {
                    console.error("zdkdropdown.selectValue(): value '" + value + "' does not exist in dropdown list named "
                            + "'" + selectElName + "'.");
                }
            } else {
                console.error('zdkdropdown.selectValue(): no option is defined for the dropdown list named ' 
                        + "'" + selectElName + "'.");
            }
        }
    });
