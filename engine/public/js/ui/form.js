...
$.widget("znetdk.zdkform", {
    ...
    _create: function () {
        ...
        this.formErrorMsgArea = $('<p class="zdk-form-error-msg ui-helper-hidden ui-state-error ui-corner-all"/>').prependTo(this.element);
        this.formErrorMsgArea.append('<span class="fa fa-warning fa-lg"></span>');
            ...
            for (property in values) {
                ...
                if (inputElement.length) {
                    if (inputElement.attr("type") === "checkbox") {
                        ...
                        if ($.type(selectedOptions) === 'array') {
                            inputElement.zdklistbox('selectItemsByValues', selectedOptions);
                        } else if ($.type(selectedOptions) === 'string') {
                            inputElement.zdklistbox('selectItemByValue', selectedOptions);
                        }
    ...
    /**
     * Set the access of the input elements (user keybord input enabled or
     * disabled).
     * @param {boolean} readOnly Value true for setting the form input elements
     * in read only access. Otherwise value false for allowing the user to enter
     * data into the input elements.
     */
    setReadOnly: function(readOnly) {
        var widgetState = readOnly === true ? 'disable' : 'enable';
        this.element.find("input[type!='hidden'],textarea,select,.zdk-tree,."
                + "zdk-inputhtml,.zdk-multiupload,.zdk-inputhtml,.zdk-inputrows").each(function () {
            if ($(this).attr("type") === "checkbox") { // Checkboxes
                $(this).puicheckbox(widgetState);
            } else if ($(this).attr("type") === "radio") { // Radio buttons
                $(this).puiradiobutton(widgetState);
            } else if ($(this).attr("type") === "file") { // File Inputs
                // Not yet supported
            } else if ($(this).hasClass('zdk-date')) { // Date Inputs
                $(this).zdkinputdate(widgetState);
            } else if ($(this).hasClass('zdk-autocomplete')) { // Autocomplete Inputs
                $(this).zdkautocomplete(widgetState);
            } else if ($(this).hasClass('zdk-dropdown')) { // Dropdown widgets
                $(this).zdkdropdown(widgetState);
            } else if ($(this).hasClass('zdk-listbox')) { // Listbox widgets
                $(this).zdklistbox(widgetState);
            } else if ($(this).hasClass('zdk-tree')) { // Tree widgets
                // Not yet supported
            } else if ($(this).hasClass('zdk-multiupload')) { // Multi upload widgets
                $(this).zdkmultiupload(widgetState);
            } else if ($(this).hasClass('zdk-inputhtml')) { // HTML inputs
                $(this).zdkinputhtml(widgetState);
            } else if ($(this).hasClass('zdk-inputrows')) { // Input rows widget
                $(this).zdkinputrows(widgetState);
            } else if ($(this).is('textarea') && $(this).hasClass('pui-inputtext')) { // Textarea Inputs
                $(this).prop('readonly', readOnly === true);
            } else if ($(this).is('input') && $(this).hasClass('pui-inputtext')) { // Text Inputs
                $(this).prop('readonly', readOnly === true);
            }
        });
    },
    ...
    getFormData: function (getEmptyInputs) {
        ...
        this.element.find("input[type!='radio'],textarea,select,.zdk-tree,.zdk-picklist,.zdk-radiobuttongroup,.zdk-inputhtml,button:submit:focus")
            ...
            if (elementName !== "" && elementName !== undefined) {
                ...
                // Date values are returned in W3C format
                    ...
                    } else if (selectedOptions !== null && $.type(selectedOptions) === 'string') {
                        // Unique selection for listbox
                        formData.push({name: elementName, value: selectedOptions});
                    }
                ...
                // Selected nodes of tree components
                ...
                } else if ($(this).hasClass('zdk-picklist')) {
                    var selectedItems = $(this).zdkpicklist('getSelection');
                    if (selectedItems.length || getAll) {
                        formData = formData.concat(selectedItems);
                    }
                } else if ($(this).is(':file')) {
                    if ($(this).val() !== "" || getAll) {
                    ...
                    if ($(this).val() !== "" || getAll) {
                        ...
                        ($(this).is(':disabled') && $(this).val().trim() !== "")) {
                    if ($(this).val().trim() !== "" || getAll) {
                        formData.push({
                            name: elementName, 
                            value: $(this).hasClass('zdk-form-notrim')
    ...
    /**
     * Returns the values of the specified element
     * @param {String} elementName The name of the element 
     * @returns {array} The values of the specified element as array or null if
     * the specified element does not exist
     */
    getInputValues: function(elementName) {
        var formData = this.getFormData(true),
            values = [];
        for (i = 0; i < formData.length; i++) {
            if (formData[i].name === elementName) {
    ...
    _reset: function(event) {
        ...
        // Reset Picklist to initial state
        $this.element.find('.zdk-picklist').each(function () {
            $(this).zdkpicklist('reset');
        });
    ...
    _submitFormData: function() {
        if (this.options.controller && this.options.action) {
            ...
            var fullData = this.element.find('.zdk-inputrows').length > 0
                || this.element.attr('data-zdk-allvalues') === 'true';
    ...
    _submit: function (event) {
        var $this = event.data;
        $this.firstElementNameInError = ''; // Bug fixing, i.e wrong error ...
        // ... message could be displayed if no dropdown item is selected 
        
        if ($this._validate()) {
            $this._trigger('beforesubmit'); // Event triggered after extra input validation
