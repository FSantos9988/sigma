...
$(function () {
    $.widget("znetdk.zdkinputfile", {
        ...
        _bindEvents: function () {
            ...
            this.element.on('change.zdkinputfile_' + $(this).attr('id'), $this, function() {
                ...
                if (filesCount) {
                    ...
                    $this.getReducedPhoto(originalFile, function(reducedPhoto){
                        var selectedFile = reducedPhoto !== null ? reducedPhoto : originalFile;
        ...
        /**
         * Reduces the specified photo.
         * @param {File} file The photo to reduce
         * @param {type} callback The function to callback once the photo is
         * reduced. The new photo File is given as parameter. If reduction 
         * failed, null is given instead.
         */
        getReducedPhoto: function(file, callback) {
            if (!file.type.startsWith("image/")) {
                callback(file); // Not a photo
                return;
            }
            const status = znetdk.reducePhotoSizePercent(file, 
                this.options.photoReductionPercent,
                this.options.minPhotoSizeForReduction, callback);
            if (status === false) {
                console.warn('[zdkinputfile] Invalid parameters,\n\
                    photo reduction failed.');  
            }
        },
        /**
